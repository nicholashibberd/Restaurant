module Admin::MenuItemsHelper
  def admin_edit_navigation_breadcrumb(nav)
    index_url = link_to('Navigation', :controller => 'admin/navigation_menus', :action => 'index')    
    if nav.has_parent?
      ...
      return content_tag(:span, "#{index_url} &raquo; #{nav_url} &raquo; #{link_to(nav.parent.name, parent_url)} &raquo; #{nav.name}")      
    else
      nav_menu = NavigationMenu.find(nav.navigation_menu_id)      
      nav_url = link_to(nav_menu.name, :controller => 'admin/navigation_menus', :action => 'edit', :id => nav_menu.id)
      return content_tag(:span, "#{index_url} &raquo; #{nav_url} &raquo; #{nav.name}")
    end    
  end
  ...
  def admin_new_navigation_breadcrumb
    ...
    if params[:parent_id]
      ...
      return content_tag(:span, "#{index_url} &raquo; #{nav_url} &raquo; #{link_to(parent.name, parent_url)} &raquo; Add sub menu")      
    else
      ...
      return content_tag(:span, "#{index_url} &raquo; #{nav_url} &raquo; Add sub menu")
    end
  end
