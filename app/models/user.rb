class User < ActiveRecord::Base
  ...
  def self.authenticate(email, submitted_password, site)
    user = User.all(:conditions => {:email => email, :site_id => site}).first
     return nil if user.nil?
     return user if user.has_password?(submitted_password)
  end

  def self.authenticate_old(email, submitted_password, site)
